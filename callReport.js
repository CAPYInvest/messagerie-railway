...
async function summarizeText(text) {
 ...
 titre: chaîne, date: chaîne (JJ MMMM YYYY), heure: chaîne (HH:mm), objet: chaîne, participants: liste de chaînes, pointsCles: liste de chaînes, prochainesEtapes: liste de chaînes, actionsARealiser: liste de chaînes, conclusion: chaîne.\nTranscription :\n${text}`;
  ...
  try {
    ...
    console.warn('[callReport] JSON parsing failed, using fallback');
    return {
      titre: 'Compte Rendu',
      date: new Date().toLocaleDateString('fr-FR', { day:'2-digit', month:'long', year:'numeric' }),
      heure: new Date().toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'}),
      objet: '', participants: [], pointsCles: [], prochainesEtapes: [], actionsARealiser: [],
      conclusion: text
    };
  }
}
...
router.post('/', async (req, res) => {
  try {
    ...
    const bufTrans = await Packer.toBuffer(doc);
