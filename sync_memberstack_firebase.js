...
// Vérification de l'initialisation de Firebase Admin.
// Si aucune instance n'existe, on initialise en utilisant la configuration stockée dans la variable d'environnement Railway.
...
const memberstackClient = axios.create({
  baseURL: 'https://admin.memberstack.com', // URL de base définie dans la documentation Memberstack
  headers: {
    'X-API-KEY': API_KEY,
...
// Intercepteur pour gérer les erreurs et les limitations de taux (rate limiting)
memberstackClient.interceptors.response.use(
  ...
  async error => {
    if (error.response && error.response.status === 429) {
      ...
      await new Promise(resolve => setTimeout(resolve, 2000)); // Attente de 2 secondes avant de retenter
...
/**
 * Endpoint POST déclenché lors d'une nouvelle inscription depuis Webflow.
 * L'endpoint reçoit les données du formulaire, puis appelle l'API Memberstack afin
 * d'obtenir les informations complètes du membre pour synchroniser Firestore.
 */
router.post('/sync/member', async (req, res) => {
  try {
    const memberData = req.body;
    
    // Validation minimale : l'email est requis.
    if (!memberData.email) {
    ...
    if (memberData.id) {
      // Si l'ID du membre est fourni, récupérer les informations complètes via l'API Memberstack
      const response = await memberstackClient.get(`/members/${memberData.id}`);
      memberFromApi = response.data;
    } else {
      // Sinon, pour cet exemple, nous utilisons directement les données fournies.
      // Selon votre implémentation, vous pourriez appeler un endpoint de recherche par email.
      memberFromApi = memberData;
    }
    
    // Vérifie que les données indispensables sont présentes
...
/**
 * Endpoint GET pour déclencher une synchronisation globale de tous les membres.
 * Accessible via : https://votre-domaine.com/api/sync/members
 */
